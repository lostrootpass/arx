...
INTERACTIVE_OBJ * AddFix(LPDIRECT3DDEVICE7 pd3dDevice, char * file, long flags)
{
	...
	SendConsole(texx, 2, 0, (HWND)g_pRenderApp->m_hWnd);
...
void RenderInterGL(float from, float to, long flags)
{
	float val = -0.6f;
	EERIE_3D temp;
	EERIEMATRIX mat;
					...
					|| (EDITMODE))
				{
					if (io->obj)
					{
						...
						if ((io->obj->pbox)
							&& (io->obj->pbox->active))
						{
							DrawEERIEInterMatrix(0, io->obj, &mat, &io->pos, io, NULL);
						}
						else
						{
							DrawEERIEInter(0, io->obj, &temp, &io->pos, io);
						}
