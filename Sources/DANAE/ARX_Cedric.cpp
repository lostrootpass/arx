...
void	Cedric_RenderObjectGL(EERIE_3DOBJ * eobj, EERIE_C_DATA * obj, INTERACTIVE_OBJ * io, EERIE_3D * pos, EERIE_3D & ftr, float invisibility)
{
	if (bRenderInterList)
	{
	...
	if ((io) && (io->halo.flags & HALO_ACTIVE))
	{
		...
		{
			Cedric_PrepareHalo(0, eobj, obj, io, pos, ftr);
			need_halo = 1;
		}
	}
...
void	Cedric_AnimateDrawEntityGL(EERIE_3DOBJ * eobj,
	ANIM_USE * animuse,
	EERIE_3D * angle,
	EERIE_3D * pos,
	INTERACTIVE_OBJ * io,
	...
	long typ)
{
	...
	if(Cedric_IO_Visible(io, typ))
	{
		...
		if((Cedric_TransformVerts(io, eobj, obj, pos, angle)) && (!(typ & ANIMQUATTYPE_NO_RENDER)))
		{
			...
			Cedric_RenderObjectGL(eobj, obj, io, pos, ftr, invisibility);
			...
			for(long k = 0; k < eobj->nblinked; k++)
			{
				...
				if((eobj->linked[k].lgroup != -1) && eobj->linked[k].obj)
				{
					...
					DrawEERIEInterMatrix(0, obj, &matrix, posi, ioo, NULL, &eobj->linked[k].modinfo);
					INVISIBILITY_OVERRIDE = 0.f;
